...
class Transaction(db.Model):
    ...
    transactions_ba = db.relationship(
        'Transaction_bank_account', backref=db.backref('Transaction', uselist=True))
    transactions_re = db.relationship(
        'Transaction_real_estate', backref=db.backref('Transaction', uselist=True))
    transactions_bond = db.relationship(
        'Transaction_bond', backref=db.backref('Transaction', uselist=True))
    transactions_stock = db.relationship(
        'Transaction_stock', backref=db.backref('Transaction', uselist=True))
    transactions_debt = db.relationship(
        'Transaction_debt', backref=db.backref('Transaction', uselist=True))
    transaction_tags = db.relationship(
        'Transaction_tag', backref=db.backref('Transaction', uselist=True))
...
class Bank_account(db.Model):
    ...
    # relationships
    transactions_ba = db.relationship(
        'Transaction_bank_account', backref=db.backref('Bank_account', uselist=False))
...
class Transaction_bank_account(db.Model):
    ...
    transaction_id = db.Column(db.Integer, db.ForeignKey(
        Transaction.transaction_id), nullable=False)
    bank_account_id = db.Column(db.Integer, db.ForeignKey(
        Bank_account.bank_account_id), nullable=False)
...
class Debt(db.Model):
    ...
    # relationships
    transactions_debt = db.relationship(
        'Transaction_debt', backref=db.backref('Debt', uselist=False))
...
class Real_estate(db.Model):
    ...
    # relationships
    transactions_re = db.relationship(
        'Transaction_real_estate', backref=db.backref('Real_estate', uselist=False))
...
class Transaction_real_estate(db.Model):
    ...
    transaction_id = db.Column(db.Integer, db.ForeignKey(
        Transaction.transaction_id), nullable=False)
    real_estate_id = db.Column(db.Integer, db.ForeignKey(
        Real_estate.real_estate_id), nullable=False)
...
class Bond(db.Model):
    ...
    # relationships
    transactions_bond = db.relationship(
        'Transaction_bond', backref=db.backref('Bond', uselist=False))
...
class Stock(db.Model):
    ...
    # relationships
    transactions_stock = db.relationship(
        'Transaction_stock', backref=db.backref('Stock', uselist=False))
...
def database_test():
    ...
    account_test = Account(user_id=user_selected.user_id,
                           account_name='tester', account_type='Savings')
    # print(Account.query.filter_by(account_type='Savings').first())
    db.session.add(account_test)
    db.session.commit()
    account_test = Account(user_id=user_selected.user_id,
                           account_name='tester2', account_type='Savings')
    # print(Account.query.filter_by(account_type='Savings').first())
    db.session.add(account_test)
    db.session.commit()  
    ...
    transaction_test = Transaction(
        account_id=account_selected.account_id, transaction_type='Cash', transaction_value=50)
    # print(Transaction.query.filter_by(account_id=1).first())
    db.session.add(transaction_test)
    db.session.commit()
    transaction_test = Transaction(
        account_id=account_selected.account_id, transaction_type='Debt', transaction_value=50)
    # print(Transaction.query.filter_by(account_id=1).first())
    db.session.add(transaction_test)
    db.session.commit()
    transaction_test = Transaction(
        account_id=account_selected.account_id, transaction_type='Bond', transaction_value=50)
    # print(Transaction.query.filter_by(account_id=1).first())
    db.session.add(transaction_test)
    db.session.commit()
    transaction_test = Transaction(
        account_id=account_selected.account_id, transaction_type='Stock', transaction_value=50)
    # print(Transaction.query.filter_by(account_id=1).first())
    db.session.add(transaction_test)
    db.session.commit()
    transaction_test = Transaction(
        account_id=account_selected.account_id, 
    ...
    bank_account_trans_test = Transaction_bank_account(
        transaction_id=transaction_selected_cash.transaction_id, 
        bank_account_id=bank_account_selected.bank_account_id)
    ...
    bank_account_trans_selected = Transaction_bank_account.query.filter_by(
        bank_account_id=1).first()
    ...
    bond_trans_test = Transaction_bond(
        transaction_id=transaction_selected_bond.transaction_id, 
        bond_id=bond_selected.bond_id)
    ...
    stock_trans_test = Transaction_stock(
        transaction_id=transaction_selected_stock.transaction_id, 
    ...
    re_trans_test = Transaction_real_estate(
        transaction_id=transaction_selected_re.transaction_id, 
        real_estate_id=real_estate_selected.real_estate_id)
    ...
    #debt
    debt_test = Debt(lender='Jersey Boys', principal=60, interest_rate=2, interest_period='Month',
                     payment_account=account_selected.account_id, payment_date='Weekly')
    ...
    debt_trans_test = Transaction_debt(
        transaction_id=transaction_selected_debt.transaction_id, debt_id=debt_selected.debt_id)
    ...
    test_trans_tag = Transaction_tag(
        transaction_id=transaction_selected_cash.transaction_id, 
    ...
    account_tag_test = Account_tag(
        account_id=account_selected.account_id, tag_id=tag_selected_two.tag_id)
    ...
    join_test = db.session.query(Account, Transaction, Transaction_bank_account)
        .filter_by(account_id=account_selected.account_id)
        .join(Transaction)
        .join(Transaction_bank_account)
        .first()
    #print(join_test)
    #print(join_test.Transaction.transaction_id)
    sum_test = db.session.query(func.sum(Transaction.transaction_value).label(
        'balance')).filter_by(transaction_type='Cash').first()
    sum_test = db.session.query(func.sum(Transaction.transaction_value).label(
        'balance')).filter(Transaction.transaction_id <= 12).first()
